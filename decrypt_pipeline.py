...
# style_img = image_loader("./style_library/van_gogh.webp")
...
def save_im(inp_im, fname, parent_path):
    im = inp_im.detach().squeeze().cpu().numpy() * 255
    im = im.astype(np.uint8)
    im = np.transpose(im, (1, 2, 0))
    im = Image.fromarray(im)
    im.save(f"{parent_path}{fname}.jpg")

# %%
# we know T, and now we find Y from Y'
# style_img = T
# input_img = Y'
